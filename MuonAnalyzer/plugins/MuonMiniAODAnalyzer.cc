...
void MuonMiniAODAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& tag : tag_muon_ttrack) {
	...
	if (vtx.prob() < minSVtxProb_)
      ...
      FillMiniIsov2<pat::Muon, pat::PackedCandidate>(*pfcands, tag.first, *rhoHandle, nt, true);
      ...
      if (it != trk_muon_map.first.end()) {
        ...
        FillMiniIsov2<pat::Muon, pat::PackedCandidate>(
            *pfcands, muons->at(trk_muon_map.second[idx]), *rhoHandle, nt, false);
        if (includeJets_)
        ...
        FillMiniIsov2<pat::Muon, pat::PackedCandidate>(*pfcands, fakeMuon, *rhoHandle, nt, false);
        if (includeJets_)
